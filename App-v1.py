...
prep_level_display = st.selectbox("Prep Work Complexity", ["Low (minimal prep)", "Medium (some scraping/patching)", "High (extensive prep)"])
crew_size = st.slider("Crew Size (number of painters)", min_value=1, max_value=4, value=2)

# Map display prep level to dictionary key
prep_level_map = {
    "Low (minimal prep)": "Low",
    "Medium (some scraping/patching)": "Medium",
    "High (extensive prep)": "High"
}
prep_level = prep_level_map.get(prep_level_display, "Medium")  # Default to Medium if mapping fails
...
def calculate_hours(sq_ft, scope, prep_level):
    try:
        ...
        st.error(f"Error in hour calculation: Invalid scope ({scope}) or prep level ({prep_level}). Please check inputs.")
        return 0
